...
def visualize_deformation(U, node_positions, scale=1.0):
    """
    ...
    fig.suptitle(r"Raw $U_g (x)$", fontsize=16, fontweight='bold')
    
    color = "#4F81BD"  # Consistent blue color for all plots
    
    # Pairing u_y - x with theta_z - x, and u_z - x with theta_y - x
    deformation_pairs = [
    ...
    for i, (ax_left, ax_right, (disp, disp_label, rot, rot_label)) in enumerate(zip(axes[:, 0], axes[:, 1], deformation_pairs)):
        ax_left.plot(node_positions, disp * 1000, color=color, linewidth=2, marker='o', markerfacecolor=color, markeredgecolor=color)
        ...
        ax_right.plot(node_positions, np.degrees(rot), color=color, linewidth=2, marker='o', markerfacecolor=color, markeredgecolor=color)
...
def process_deformation_files():
    ...
    for file_path in files:
        ...
        if U is not None and mesh_dictionary is not None and 'node_coordinates' in mesh_dictionary:
            ...
            visualize_deformation(U, node_positions, scale=1.0)
