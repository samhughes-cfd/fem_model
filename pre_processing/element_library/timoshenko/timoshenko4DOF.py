...
class TimoshenkoBeamElement4DOF(pre_processing.element_library.element_1D_base.Element1DBase):
    """
    Timoshenko beam element (1D) with 4 DOFs per node:
    (u_x, u_y, θ_y, θ_z), where:
        - θ_z accounts for bending effects.
        - θ_y accounts for transverse shear deformation.
    ...
    def strain_displacement_matrix(self, dN_dxi: np.ndarray) -> tuple:
        ...
        Returns:
            tuple: (B, B_axial, B_bending, B_shear)
                - `B` (ndarray): Combined strain-displacement matrix (3x4).
        ...
        B_axial[3] = dN_dxi[3] * dxi_dx  # Axial strain from θ_y

        B_bending = np.zeros(4)
        B_bending[1] = dN_dxi[1] * (dxi_dx**2)  # Bending strain contribution
        B_bending[2] = dN_dxi[2] * dxi_dx       # Bending moment from θ_z

        B_shear = np.zeros(4)
        B_shear[1] = dN_dxi[1] * dxi_dx         # Shear strain contributes to θ_y
        B_shear[2] = dN_dxi[2] * dxi_dx         # Shear strain contributes to θ_z
    ...
    def material_stiffness_matrix(self) -> tuple:
        ...
        D_shear = self.G * self.ks * self.A  # Shear stiffness applied to θ_y and θ_z

        D = np.diag([D_axial, D_bending + D_shear, D_shear])  # θ_y and θ_z stiffness include shear
    ...
    def element_stiffness_matrix(self):
        ...
        dof_map = [0, 1, 2, 6, 7, 8]  # Axial, transverse, θ_y, and θ_z DOFs per node
    ...
    def element_force_vector(self):
        ...
        dof_map = [0, 1, 2, 6, 7, 8]  # u_x, u_y, θ_y, θ_z
