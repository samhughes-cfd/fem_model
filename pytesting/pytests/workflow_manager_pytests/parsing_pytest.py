...
# === Dynamically Add workflow_manager to Path ===
def find_project_root():
    """Finds the project root by looking for a marker file or falling back to PYTHONPATH."""
    current_path = os.path.abspath(__file__)
    while not os.path.exists(os.path.join(current_path, ".project_root")) and os.path.dirname(current_path) != current_path:
...
def ensure_dependencies():
    ...
    try:
        ...
        pytest.fail("Missing dependency: NumPy is required for tests.")
...
def job_dirs_fixture():
    ...
    if not job_dirs:
        logger.warning(f"⚠️ No job_* directories found in {jobs_dir}. Falling back to current directory.")
        job_dirs = [os.getcwd()]
    return job_dirs
